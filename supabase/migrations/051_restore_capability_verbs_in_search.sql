-- ─────────────────────────────────────────────────────────────────────────────
-- 051_restore_capability_verbs_in_search.sql
--
-- Forward patch for 050. The short generic timeout guard worked for "send
-- email", but its trigram stoplist was too broad for a tool registry: words
-- like fetch, search, query, read, write, post, and check are often the actual
-- capability being requested, not disposable verbs.
--
-- This keeps the same search document table and indexes. No new DB bloat.
-- Changes:
--   1. Disable loose OR FTS for one/two-token queries.
--   2. Use only significant capability/provider tokens for trigram name/tool lanes.
...
AS $$
  ...
  significant_words AS (
    ...
    FROM raw_words
    WHERE word NOT IN (
      'add', 'append', 'call', 'create', 'delete', 'find',
      'get', 'list', 'make', 'manage', 'run',
      'send', 'set', 'sync', 'update', 'use'
    )
  ),
  strict_fts AS (
    SELECT
...
COMMENT ON FUNCTION public.search_servers IS
  '051: Restores capability verbs in trigram lanes while preserving short generic timeout guard.';
