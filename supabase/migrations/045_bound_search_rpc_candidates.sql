-- ─────────────────────────────────────────────────────────────────────────────
-- 045_bound_search_rpc_candidates.sql
--
-- Goal: keep search_servers() below Supabase statement timeouts without
-- dropping indexes or removing tool text trigram search.
--
-- The 041/043 RPC used one broad OR over server_tools:
--   FTS OR tool-name similarity OR full-document text trigram
-- followed by GROUP BY across every matching tool row. Broad action intents
-- such as "send a transactional email with html body" can make that plan too
-- expensive on a large catalog.
--
-- This version preserves the public RPC contract and keeps the
-- idx_server_tools_text_trgm strategy, but splits tool matching into bounded
-- candidate lanes before aggregation:
--   1. FTS over tool search_vector
--   2. trigram over short tool_name
--   3. trigram over full search_text, capped before GROUP BY
-- ─────────────────────────────────────────────────────────────────────────────

CREATE INDEX IF NOT EXISTS idx_server_tools_server_id
  ON public.server_tools(server_id);
...
AS $$
  ...
  server_candidates AS (
    SELECT
      ...
      GREATEST(
        ...
        similarity(coalesce(s.display_name, ''), q.raw)
      ) AS name_sim,
      row_number() OVER (
        ORDER BY
          ts_rank_cd(s.search_vector, q.tsq, 32) DESC,
          GREATEST(similarity(s.name, q.raw), similarity(coalesce(s.display_name, ''), q.raw)) DESC,
          s.trust_score DESC
      ) AS server_pos
  ...
  tool_fts_rows AS (
    SELECT
      st.server_id,
      st.server_name,
      ts_rank_cd(st.search_vector, q.tsq, 32) AS tool_rank,
      similarity(st.tool_name, q.raw) AS tool_name_sim,
      0.0::FLOAT8 AS text_hit_bonus
    FROM public.server_tools st
    CROSS JOIN q
    WHERE q.raw <> ''
      AND st.search_vector @@ q.tsq
    ORDER BY ts_rank_cd(st.search_vector, q.tsq, 32) DESC
    LIMIT GREATEST(result_limit * 80, 400)
  ),
  tool_name_rows AS (
    SELECT
      st.server_id,
      st.server_name,
      0.0::FLOAT8 AS tool_rank,
      similarity(st.tool_name, q.raw) AS tool_name_sim,
      0.02::FLOAT8 AS text_hit_bonus
    FROM public.server_tools st
    CROSS JOIN q
    WHERE q.raw <> ''
      AND st.tool_name % q.raw
  ...
  tool_text_rows AS (
    SELECT
      ...
      0.0::FLOAT8 AS tool_rank,
      similarity(st.tool_name, q.raw) AS tool_name_sim,
      0.05::FLOAT8 AS text_hit_bonus
    FROM public.server_tools st
    CROSS JOIN q
    WHERE q.raw <> ''
      AND st.search_text % q.raw
    LIMIT GREATEST(result_limit * 40, 200)
  ),
  tool_rows AS (
    SELECT * FROM tool_fts_rows
    UNION ALL
    SELECT * FROM tool_name_rows
    UNION ALL
    SELECT * FROM tool_text_rows
  ),
  tool_candidates AS (
    SELECT
      tr.server_name,
      max(tr.tool_rank) AS tool_rank,
      max(tr.tool_name_sim) + max(tr.text_hit_bonus) AS tool_sim,
      row_number() OVER (
        ORDER BY
          max(tr.tool_rank) DESC,
          max(tr.tool_name_sim) + max(tr.text_hit_bonus) DESC
      ) AS tool_pos
    FROM tool_rows tr
    JOIN public.servers s ON s.id = tr.server_id
    WHERE
      s.status = 'active'
      AND (include_stdio OR COALESCE(s.transport, 'unknown') != 'stdio')
    GROUP BY tr.server_name
    LIMIT GREATEST(result_limit * 40, 200)
  ),
  combined_names AS (
...
COMMENT ON FUNCTION public.search_servers IS
  '045: Bounded tool candidate lanes for timeout-resistant tool-level search while preserving text trigram matching.';
