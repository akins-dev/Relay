-- ─────────────────────────────────────────────────────────────────────────────
-- 043_restore_tool_text_trigram_search.sql
--
-- Goal: restore full-document tool trigram matching after 042.
--
-- The text trigram index remains part of the search strategy. This migration
-- puts search_servers() back on the 041 behavior while preserving forward-only
-- migration history for databases where 042 was already applied.
-- ─────────────────────────────────────────────────────────────────────────────
...
COMMENT ON FUNCTION public.search_servers IS
  '043: Restores tool text trigram matching in search_servers after 042.';
