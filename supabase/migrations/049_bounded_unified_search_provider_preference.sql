-- ─────────────────────────────────────────────────────────────────────────────
-- 049_bounded_unified_search_provider_preference.sql
--
-- Fixes the 048 timeout path by replacing the broad single-pass OR predicate
-- with bounded candidate lanes over server_search_docs.
--
-- Also adds an explicit-provider preference: when a query token exactly matches
-- a server name, that server receives a bounded relevance boost. This makes
-- named-provider intents such as "GitHub issue", "Notion page", "Slack
-- message", or "Brave search" prefer the provider server over a community
-- wrapper that merely has matching tool words.
-- ─────────────────────────────────────────────────────────────────────────────
...
AS $$
  ...
  words AS (
    ...
    FROM q, unnest(string_to_array(q.raw, ' ')) AS w
  ...
  name_rows AS (
    ...
    FROM public.server_search_docs d
    CROSS JOIN q
    JOIN words w ON d.server_name % w.word OR d.server_name % q.raw
  ...
  tool_rows AS (
    ...
    FROM public.server_search_docs d
    JOIN words w ON d.tool_names % w.word
    WHERE (include_stdio OR COALESCE(d.transport, 'unknown') != 'stdio')
    GROUP BY d.server_id
...
COMMENT ON FUNCTION public.search_servers IS
  '049: Bounded unified search lanes with exact named-provider preference.';
