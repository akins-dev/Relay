-- ─────────────────────────────────────────────────────────────────────────────
-- 042_remove_tool_text_trigram_from_search.sql
--
-- Goal: temporarily remove full-document tool trigram matching from
-- search_servers() to test whether it is responsible for statement timeouts.
--
-- This migration keeps the same public search_servers contract but no longer
-- uses server_tools.search_text with the pg_trgm `%` operator. Tool documents
-- still match through indexed FTS; fuzzy trigram matching is limited to the
-- short tool_name column.
-- ─────────────────────────────────────────────────────────────────────────────
...
AS $$
  ...
  tool_candidates AS (
    SELECT
      ...
      max(similarity(st.tool_name, q.raw)) AS tool_sim,
      row_number() OVER (
        ORDER BY
...
COMMENT ON FUNCTION public.search_servers IS
  '042: Tool-level intent search avoids trigram matching over large tool documents.';
