...
const TOKEN_SECRET = (() => {
  ...
  if (!s && process.env.NODE_ENV === "production") {
    ...
    throw new Error(
      "[registry] TOKEN_SECRET or CRON_SECRET must be set in production",
    );
  }
...
function dnsLruGet(
  host: string,
): { ok: boolean; checkedAt: number } | undefined {
...
function dnsLruSet(
  host: string,
  value: { ok: boolean; checkedAt: number },
): void {
...
export async function isSafeUrlForServerFetch(url: string): Promise<boolean> {
  try {
    ...
    const ok =
      records.length > 0 &&
      records.every((record) => !isPrivateIpLiteral(record.address));
...
const ALLOWED_ORIGINS = new Set([
  process.env.NEXT_PUBLIC_APP_URL ?? "https://mcp-relay.vercel.app",
  "http://localhost:3000",
  "http://localhost:3001",
]);
...
export function corsHeaders(
  ...
  return {
    ...
    "Access-Control-Allow-Headers":
      "Content-Type, Authorization, X-Confirm-Token",
