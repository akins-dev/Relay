...
export default function DashboardPage() {
  ...
  async function createKey() {
    ...
    const res = await fetch('/api/auth/api-keys', { method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` }, body: JSON.stringify({ name: keyName }) }).then(r => r.json());
    setCreatedKey(res.key);
    setKeyName('');
    fetch('/api/auth/me', { headers: { Authorization: `Bearer ${token}` } }).then(r => r.json()).then(setData);
  }

  async function deleteKey(id: string) {
    await fetch(`/api/auth/api-keys?id=${id}`, { method: 'DELETE', headers: { Authorization: `Bearer ${token}` } });
    fetch('/api/auth/me', { headers: { Authorization: `Bearer ${token}` } }).then(r => r.json()).then(setData);
  }

  if (!data) return <div style={{ display: 'flex', justifyContent: 'center', padding: '80px' }}><div className="anim-spin" style={{ width: '24px', height: '24px', border: '2px solid var(--border)', borderTopColor: 'var(--green)', borderRadius: '50%' }} /></div>;
    ...
    <div className="page" style={{ maxWidth: '960px' }}>
      ...
      <div style={{ borderBottom: '1px solid var(--border)', marginBottom: '28px', display: 'flex' }}>
        {(['servers','keys','account'] as const).map(t => (
          <button key={t} onClick={() => setTab(t)} style={{ background: 'none', border: 'none', borderBottom: tab === t ? '2px solid var(--green)' : '2px solid transparent', color: tab === t ? 'var(--text)' : 'var(--text-3)', padding: '10px 18px', cursor: 'pointer', fontSize: '13px', fontFamily: 'var(--font)', fontWeight: tab === t ? 600 : 400, textTransform: 'capitalize' }}>
        ...
        <div>
          {createdKey && (
            <div style={{ padding: '16px', background: 'var(--green-bg)', border: '1px solid #166534', borderRadius: 'var(--radius)', marginBottom: '18px' }}>
              <div style={{ fontSize: '12px', color: 'var(--green)', marginBottom: '8px', fontWeight: 600 }}>✓ Copy this key now — it won't be shown again</div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
            {data.apiKeys?.length === 0 ? <div style={{ textAlign: 'center', padding: '40px', color: 'var(--text-3)', fontSize: '14px' }}>No API keys yet.</div> :
              data.apiKeys?.map((key: any) => (
                <div key={key.id} className="card" style={{ padding: '14px 20px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  ...
                  <div style={{ display: 'flex', gap: '10px', alignItems: 'center' }}>
                    <span style={{ fontSize: '11px', color: 'var(--text-3)' }}>{key.last_used_at ? `Used ${new Date(key.last_used_at * 1000).toLocaleDateString()}` : 'Never used'}</span>
      ...
      {tab === 'account' && (
        <div className="card" style={{ padding: '24px', maxWidth: '440px' }}>
          ...
          {[['Username', data.user.username], ['Email', data.user.email], ['Member since', new Date(data.user.created_at * 1000).toLocaleDateString()]].map(([l, v]) => (
          ...
          <button onClick={logout} className="btn btn-danger" style={{ marginTop: '24px', width: '100%', justifyContent: 'center' }}>Sign out</button>
        </div>
      )}
    </div>
  );
