...
export default function ServerDetailPage() {
  ...
  useEffect(() => {
    fetch(`/api/servers/${name}`, { headers: token ? { Authorization: `Bearer ${token}` } : {} })
      .then(r => r.json())
  ...
  async function toggleStar() {
    ...
    const res = await fetch(`/api/servers/${server.name}/star`, { method: 'POST', headers: { Authorization: `Bearer ${token}` } }).then(r => r.json());
  ...
  if (!server) return <div className="page-sm" style={{ textAlign: 'center', paddingTop: '80px', color: 'var(--text-2)' }}>Server not found. <Link href="/registry" style={{ color: 'var(--green)' }}>Back to registry</Link></div>;
    ...
    <div className="page" style={{ maxWidth: '960px' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '28px', flexWrap: 'wrap', gap: '16px' }}>
        <div>
          ...
          <div style={{ display: 'flex', gap: '6px', marginTop: '12px', flexWrap: 'wrap' }}>
            {server.tags.map(t => <Link key={t} href={`/registry?tag=${t}`} style={{ textDecoration: 'none' }}><span className="tag tag-default">{t}</span></Link>)}
          </div>
        </div>
        <div style={{ display: 'flex', gap: '8px' }}>
          <button onClick={toggleStar} className={`btn ${starred ? 'btn-primary' : 'btn-ghost'}`}>★ {server.stars?.toLocaleString()}</button>
          {server.github_url && <a href={server.github_url} target="_blank" rel="noopener" className="btn btn-ghost" style={{ textDecoration: 'none' }}>GitHub →</a>}
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5,1fr)', gap: '1px', background: 'var(--border)', border: '1px solid var(--border)', borderRadius: 'var(--radius-lg)', overflow: 'hidden', marginBottom: '32px' }}>
        {[
          ...
          { label: 'Uptime', value: `${server.uptime_pct?.toFixed(2) ?? '—'}`, unit: '%' },
          ...
          <div key={s.label} style={{ background: 'var(--bg-1)', padding: '16px 20px', textAlign: 'center' }}>
            <div style={{ fontSize: '22px', fontWeight: 800, fontFamily: 'var(--mono)', color: s.color ?? 'var(--text)', letterSpacing: '-0.02em' }}>
      ...
      <div style={{ borderBottom: '1px solid var(--border)', marginBottom: '28px', display: 'flex' }}>
        {(['overview','tools','security','integrate'] as const).map(t => (
          <button key={t} onClick={() => setTab(t)} style={{ background: 'none', border: 'none', borderBottom: tab === t ? '2px solid var(--green)' : '2px solid transparent', color: tab === t ? 'var(--text)' : 'var(--text-3)', padding: '10px 18px', cursor: 'pointer', fontSize: '13px', fontFamily: 'var(--font)', fontWeight: tab === t ? 600 : 400, textTransform: 'capitalize', transition: 'color .15s' }}>{t}</button>
        ))}
      </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 270px', gap: '20px' }}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
            ...
            <div className="card" style={{ padding: '22px' }}>
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px' }}>
                {server.tools.map(t => <span key={t} style={{ background: 'var(--bg-3)', border: '1px solid var(--border)', borderRadius: '6px', padding: '4px 12px', fontFamily: 'var(--mono)', fontSize: '12px', color: 'var(--green)' }}>fn {t}()</span>)}
              </div>
            </div>
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
            <div className="card" style={{ padding: '18px' }}>
              ...
              {[['Author', server.author_name ?? 'unknown'], ['License', server.license], ['Version', server.version]].map(([l, v]) => (
      ...
      {tab === 'security' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
          {scans.length === 0 ? <div style={{ textAlign: 'center', padding: '40px', color: 'var(--text-3)' }}>No scan data available</div> :
            scans.map((scan, i) => (
              <div key={i} className="card" style={{ padding: '20px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '10px' }}>
                  ...
                  <span style={{ fontSize: '11px', color: 'var(--text-3)', fontFamily: 'var(--mono)' }}>{new Date(scan.created_at * 1000).toLocaleDateString()}</span>
                </div>
