...
- Mark prototype-retired objects clearly instead of pretending they never existed.
...
| `002_seed_data.sql` | active | Initial seed data. |
...
| `011_vault_secrets.sql` | legacy | Vault-backed secret storage. Not required for the prototype loop. |
| `012_oauth_connections.sql` | legacy | OAuth connection storage. Not required for the prototype loop. |
...
| `022_analytics_intelligence_layer.sql` | legacy | Intelligence/learning tables from search-to-invoke roadmap. |
...
| `032_behavioral_trust_and_dynamic_diversity.sql` | legacy | Behavioral trust from proxy outcomes. Useful later if CLI reports outcomes. |
...
| `036_processing_jobs_and_mvp_ingest.sql` | superseded | Added async processing queue. Superseded by prototype scope. |
...
| Search | `search_servers(...)` | Agent-facing intent search. |
...
| Legacy runtime | policies, vault, OAuth, invoke outcomes, metering | Not part of prototype execution path. |

## Retired Objects

| Object | Retired By | Reason |
|---|---|---|
| `public.server_processing_jobs` | `037_drop_processing_jobs_queue.sql` | Async probe/sandbox/CVE queue is not part of the prototype. |
